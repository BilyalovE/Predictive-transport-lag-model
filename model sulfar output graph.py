...
for column in data.columns:
    column_data_1500 = data.loc[data['Координата'] == 2500, column].values
...
sulfur_data_1500 = column_arrays_1500['Сера'][:]
...
plt.subplot(3, 1, 1)
plt.plot(time_data, sulfur_data_0, linestyle='-')
plt.title('Изменение серы на входе в трубу', fontsize=12)
plt.xlabel('Время', fontsize=10)
plt.ylabel('Содержание серы, ppm', fontsize=10)
plt.grid(True)
...
plt.gca().locator_params(axis='y', nbins=15)
# Применение пользовательского числа разбиений для сетки на нижнем графике
plt.gca().locator_params(axis='x', nbins=35)
plt.tick_params(axis='both', which='major', labelsize=10) # Уменьшение размера цифр осей данных
...
plt.subplot(3, 1, 2)
plt.plot(time_data, sulfur_data_1500, linestyle='-')
plt.title('Изменение серы на выходе из трубы', fontsize=12)
plt.xlabel('Время', fontsize=10)
plt.ylabel('Содержание серы, ppm', fontsize=10)
plt.grid(True)
plt.ylim(170, max(sulfur_data_1500)+10) 
plt.xlim(0, max(time_data))  # Устанавливаем максимальное значение по оси времени
plt.xticks(rotation=45)  # Поворот подписей по оси x
plt.gca().set_xticks(time_data)  # Установка меток оси x в соответствии с данными
...
# График для расхода
plt.subplot(3, 1, 3)
plt.plot(time_data, volume_flow, linestyle='-')
plt.title('График изменения расхода', fontsize=12)
plt.xlabel('Время', fontsize=10)
plt.ylabel('Объемный расход, м3/час', fontsize=10)
plt.grid(True)
plt.ylim(90, max(volume_flow)+10) 
plt.xlim(0, max(time_data))  # Устанавливаем максимальное значение по оси времени
plt.xticks(rotation=45)  # Поворот подписей по оси x
plt.gca().set_xticks(time_data)  # Установка меток оси x в соответствии с данными
